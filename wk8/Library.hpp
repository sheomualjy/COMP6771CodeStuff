...
unsigned int Library<T,U>::remove(const T& item) {
    auto itemPos = std::find_if(this->items.begin(), this->items.end(), [&](const ItemContainer& i){
        return i.getItem() == item;
    });
    this->items.erase(itemPos);
...
void Library<T,U>::ItemContainer::RelatedWork::printItemAndDescription() {
    auto lockedPointer = relatedWorkLink.lock();
    if (lockedPointer)
        std::cout << *lockedPointer << " - " << relatedWorkDescription << std::endl;
}
